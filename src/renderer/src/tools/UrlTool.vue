...
import { ref } from 'vue'
import { ArrowDownUp, Clipboard, Eraser, Link } from '@lucide/vue'
...
<template>
  <ToolPage title="URL 编解码">
    <template #actions>
      <NButton type="primary" :loading="busy" @click="transform('url-encode')">
        <template #icon><NIcon :component="Link" /></template>编码
      </NButton>
      <NButton :disabled="busy" @click="transform('url-decode')">解码</NButton>
    ...
    <div class="editor-grid">
      <div class="editor-panel">
        ...
        <MonacoEditor v-model="input" aria-label="URL 输入编辑器" />
      </div>
      <div class="editor-panel">
        <div class="editor-label">输出</div>
        <MonacoEditor v-model="output" aria-label="URL 输出编辑器" />
      </div>
    </div>
  </ToolPage>
</template>
